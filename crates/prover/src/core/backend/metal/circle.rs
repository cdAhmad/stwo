...
use crate::core::backend::metal::m31::PackedM31;
...
impl PolyOps for MetalBackend {
    ...
    fn interpolate(
        ...
        values.data.iter_mut().for_each(|x| {
            *x *= inv;
        });
    ...
    fn eval_at_point(poly: &CirclePoly<Self>, point: CirclePoint<SecureField>) -> SecureField {
        ...
        let twiddle_lows = PackedSecureField::from_array(
            std::array::from_fn(|i| Self::twiddle_at(map_low, i))
        );
        let (map_mid, map_high) = map_high.split_at(4);
        let twiddle_mids = PackedSecureField::from_array(
            std::array::from_fn(|i| Self::twiddle_at(map_mid, i))
        );
        ...
        | {
            ...
            for (i, coeff_chunk) in coeff_chunk.array_chunks::<N_LANES>().enumerate() {
                ...
                let high_twiddle_factors = (
                    PackedSecureField::broadcast(twiddle_high) * twiddle_mids
                ).to_array();
    ...
    fn evaluate(
        ...
        if fft_log_size < MIN_FFT_LOG_SIZE {
            ...
            return CircleEvaluation::new(
                cpu_eval.domain,
                Col::<MetalBackend, BaseField>::from_iter(cpu_eval.values)
            );
        }
            ...
            unsafe {
                rfft::fft(
                    ...
                    transmute::<*mut PackedBaseField, *mut u32>(
                        values[
                            i << (fft_log_size - LOG_N_LANES)..(i + 1) <<
                                (fft_log_size - LOG_N_LANES)
                        ].as_mut_ptr()
                    ),
                    &subdomain_twiddles,
        ...
        CircleEvaluation::new(domain, BaseColumn {
            data: values,
            length: domain.size(),
        })
    }
...
fn compute_small_coset_twiddles(coset: Coset) -> TwiddleTree<MetalBackend> {
...
mod tests {
    ...
    use crate::core::backend::metal::circle::slow_eval_at_point;
    use crate::core::backend::metal::fft::{ CACHED_FFT_LOG_SIZE, MIN_FFT_LOG_SIZE };
    ...
    fn test_eval_at_point() {
        for log_size in MIN_FFT_LOG_SIZE + 1..CACHED_FFT_LOG_SIZE + 4 {
            let domain = CanonicCoset::new(log_size).circle_domain();
            let evaluation = CircleEvaluation::<MetalBackend, BaseField, NaturalOrder>::new(
                domain,
                (0..1 << log_size).map(BaseField::from).collect()
            );
    ...
    fn test_circle_poly_extend() {
        for log_size in MIN_FFT_LOG_SIZE..CACHED_FFT_LOG_SIZE + 2 {
            let poly = CirclePoly::<MetalBackend>::new(
                (0..1 << log_size).map(BaseField::from).collect()
            );
    ...
    fn test_optimized_precompute_twiddles() {
        ...
        let twiddles = MetalBackend::precompute_twiddles(coset);
