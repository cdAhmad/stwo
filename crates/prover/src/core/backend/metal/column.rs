...
impl SecureColumn {
    ...
    pub fn into_secure_column_by_coords(self) -> SecureColumnByCoords<MetalBackend> {
        if self.len() < N_LANES {
...
impl FromIterator<SecureField> for SecureColumnByCoords<MetalBackend> {
    fn from_iter<I: IntoIterator<Item = SecureField>>(iter: I) -> Self {
...
impl From<SecureColumnByCoords<MetalBackend>> for VeryPackedSecureColumnByCoords {
    fn from(value: SecureColumnByCoords<MetalBackend>) -> Self {
        Self {
            columns: value
                .columns
                .into_iter()
...
impl From<VeryPackedSecureColumnByCoords> for SecureColumnByCoords<MetalBackend> {
...
impl VeryPackedSecureColumnByCoords {
    ...
    /// Transforms a `&mut SecureColumnByCoords<MetalBackend>` to a
    /// `&mut VeryPackedSecureColumnByCoords`.
    ///
    /// # Safety
    ///
    /// The resulting pointer does not update the underlying columns' `data`'s lengths.
    pub unsafe fn transform_under_mut(value: &mut SecureColumnByCoords<MetalBackend>) -> &mut Self {
...
mod tests {
    ...
    use crate::core::backend::metal::m31::N_LANES;
    use crate::core::backend::metal::qm31::PackedQM31;
