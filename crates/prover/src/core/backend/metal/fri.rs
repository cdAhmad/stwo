...
impl FriOps for MetalBackend {
    fn fold_line(
        ...
        for vec_index in 0..1 << (log_size - 1 - LOG_N_LANES) {
            let value = {
                let twiddle_dbl = u32x16::from_array(
                    array::from_fn(|i| unsafe { *itwiddles.get_unchecked(vec_index * 16 + i) })
                );
    ...
    fn fold_circle_into_line(
        ...
        for vec_index in 0..1 << (log_size - 1 - LOG_N_LANES) {
            let value = unsafe {
                ...
                let twiddle_dbl = u32x8::from_array(
                    array::from_fn(|i| { *itwiddles.get_unchecked(vec_index * 8 + i) })
                );
