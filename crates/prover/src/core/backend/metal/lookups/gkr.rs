...
use crate::core::backend::metal::column::SecureColumn;
use crate::core::backend::metal::m31::{LOG_N_LANES, N_LANES};
...
fn next_grand_product_layer(layer: &Mle<MetalBackend, SecureField>) -> Layer<MetalBackend> {
...
fn next_logup_generic_layer(
    numerators: &Mle<MetalBackend, SecureField>,
    denominators: &Mle<MetalBackend, SecureField>,
) -> Layer<MetalBackend> {
...
fn next_logup_multiplicities_layer(
    numerators: &Mle<MetalBackend, BaseField>,
    denominators: &Mle<MetalBackend, SecureField>,
) -> Layer<MetalBackend> {
    assert!(denominators.len() > N_LANES);
    assert_eq!(numerators.len(), denominators.len());
...
///
/// Assumption: `len(denominators) > N_LANES`.
fn next_logup_singles_layer(denominators: &Mle<MetalBackend, SecureField>) -> Layer<MetalBackend> {
    assert!(denominators.len() > N_LANES);
...
fn eval_grand_product_sum(
    eq_evals: &EqEvals<MetalBackend>,
    col: &Mle<MetalBackend, SecureField>,
...
fn into_simd_layer(cpu_layer: Layer<CpuBackend>) -> Layer<MetalBackend> {
    match cpu_layer {
...
mod tests {
    ...
    fn gen_eq_evals_matches_cpu() {
        ...
        let eq_evals_simd = MetalBackend::gen_eq_evals(&y, two);
    ...
    fn grand_product_works() -> Result<(), GkrError> {
        ...
        let col = Mle::<MetalBackend, SecureField>::new(values.into_iter().collect());
    ...
    fn logup_with_generic_trace_works() -> Result<(), GkrError> {
        ...
        let numerators = Mle::<MetalBackend, SecureField>::new(numerators.into_iter().collect());
        let denominators = Mle::<MetalBackend, SecureField>::new(denominators.into_iter().collect());
    ...
    fn logup_with_multiplicities_trace_works() -> Result<(), GkrError> {
        ...
        let sum = zip(&numerators, &denominators)
            ...
            .sum::<Fraction<SecureField, SecureField>>();
        let numerators = Mle::<MetalBackend, BaseField>::new(numerators.into_iter().collect());
        let denominators = Mle::<MetalBackend, SecureField>::new(denominators.into_iter().collect());
