...
impl QuotientOps for MetalBackend {
    fn accumulate_quotients(
        ...
        for (ci, &c) in subdomain_shifts.iter().enumerate() {
            ...
            let twiddles = MetalBackend::precompute_twiddles(subdomain.half_coset);
...
fn accumulate_quotients_on_subdomain(
    ...
    [crate::core::poly::circle::CirclePoly<MetalBackend>; 4],
) {
    assert!(subdomain.log_size() >= LOG_N_LANES + 2);
    let mut values =
        unsafe { SecureColumnByCoords::<MetalBackend>::uninitialized(subdomain.size()) };
    ...
    // Extend the evaluation to the full domain.
    let extended_eval =
        unsafe { SecureColumnByCoords::<MetalBackend>::uninitialized(domain.size()) };
    ...
    let subeval_polys = values.map(|c| {
        i += 1;
        CircleEvaluation::<MetalBackend, BaseField, BitReversedOrder>::new(subdomain, c)
...
mod tests {
    ...
    fn test_accumulate_quotients() {
        ...
        let e1: BaseColumn = (0..small_domain.size())
            ...
            .collect();
        let polys = [
            CircleEvaluation::<MetalBackend, BaseField, BitReversedOrder>::new(small_domain, e0)
                .interpolate(),
            CircleEvaluation::<MetalBackend, BaseField, BitReversedOrder>::new(small_domain, e1)
                .interpolate(),
        ];
        ...
        let res = MetalBackend::accumulate_quotients(
            domain,
            &columns.iter().collect_vec(),
            random_coeff,
            &samples,
